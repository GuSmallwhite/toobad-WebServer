...
void sig_handler(int sig)
{
    ...
    int errnotemp = errno;
    int msg = sig;
    send(pipefd[1], (char *)&msg, 1, 0); //将信号值写入管道的写端
    ...
    while (!stop_server)
    {
        ...
        for (int i = 0; i < number; ++i)
        {
            ...
            if (socketfd == listenfd)
            {
                ...
                //创建定时器，设置其回调函数和超时时间，绑定定时器与用户数据，最后将其添加到链表中
            ...
            else if ((socketfd == pipefd[0]) && (event_list[i].events & EPOLLIN)) //处理信号
            {
                int sig;
                char signals[1024];
                ...
                else
                {
                    for (int i = 0; i < retmsg; ++i)
                    {
                        ...
                        {
                            timeout = true;//用timeout标记有定时任务需要处理
                            break;
                        }
            ...
            else if (event_list[i].events & EPOLLIN)//处理客户连接上接收到的数据
            {
                t_client *timer = client_data_list[socketfd]->timer;
                //客户的数据要读进来
            ...
            else if (event_list[i].events & EPOLLOUT)
            {
                ...
                if (http_user_list[socketfd]->write())
                {
                    ...
                    if (timer)
                    {
                        ...
                        time_t timenow = time(NULL);
                        timer->livetime = timenow + MAX_WAIT_TIME;
                        m_timer_list.adjust_timer(timer); //调整当前timer的位置
                    }
                    else
                    {
                        cout << "did not creat timer for socket:" << socketfd << endl;
